...
- [ ] Ler `IPaymentService`: cálculo/listagem de pagamentos do torneio, débitos pendentes por jogador (`GetPendingDebtsByPlayerAsync`), meus débitos (do usuário logado — resolver Player do user na liga), marcar pago, confirmar, confirmação em massa, recalcular. Autorização: envolvido (pagador/recebedor) marca pago; organizador confirma.
- [ ] TDD; commit `feat: add payment endpoints (tournament settlement, debts, confirm flow)`.
...
- [ ] `IPlayerService`: lista por liga, detalhe, criar/editar/remover (organizador), vincular user (se houver). `ISeasonService`: listar por liga, ativa/atual, criar, encerrar.
- [ ] TDD; commits `feat: add player endpoints` e `feat: add season endpoints`.
...
- [ ] `IRankingService`: ranking da temporada (ordenável por lucro/ROI/ITM se o service suportar — senão devolver o DTO completo e o front ordena), ranking geral acumulado, stats do jogador (`PlayerStats`). `IPrizeTableService`: CRUD de tabelas de premiação da liga.
- [ ] TDD; commit `feat: add ranking and prize-table endpoints`.
...
- [ ] `IJackpotService`: saldo + extrato (entradas/saídas), registrar gasto, usar em torneio (organizador). `ITournamentExpenseService`: CRUD de despesas do torneio + shares (`UpdateExpenseAsync` pattern do CLAUDE.md).
- [ ] TDD; commit `feat: add jackpot and expense endpoints`.

### Task B6: Smoke final do stream

- [ ] `dotnet build` + `dotnet test` (TODOS os projetos de teste) verdes no worktree.
- [ ] Subir a API local do worktree (`ASPNETCORE_URLS=http://localhost:5101 dotnet run --project src/PokerHub.Api --no-build` — porta 5101 para não colidir com o front stream) e conferir `/openapi/v1.json` contém os novos grupos; derrubar.
- [ ] Atualizar `docs/superpowers/plans/2026-06-11-fase2-api-endpoints.md` (este arquivo) marcando checkboxes; commit `docs: mark fase2 endpoint tasks done`.
- [ ] NÃO mergear na develop — o controller coordena o merge após o gate do front.
